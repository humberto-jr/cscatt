...
#define FORMAT "%d  %d  %f   %f\n"

static int cpu_id, max_cpu;
...
void mpi_print(const int thread_id,
               const int max_task, const double R, const double wall_time)
{
	static int counter = 0;

	if (cpu_id == 0)
	{
		printf(FORMAT, cpu_id, thread_id, R, wall_time);
		++counter;
	...
	else
	{
		#if defined(USE_MPI)
		{
			...
			sprintf(log, FORMAT, cpu_id, thread_id, R, wall_time);
...
int main(int argc, char *argv[])
{
	...
	const int rovib_grid_size = (int) file_get_key(stdin, "rovib_grid_size", 1.0, INF, 500.0);
	const int scatt_grid_size = (int) file_get_key(stdin, "scatt_grid_size", 1.0, INF, 500.0);

	const double r_min = file_get_key(stdin, "r_min", 0.0, INF, 0.5);
	const double R_min = file_get_key(stdin, "R_min", 0.0, INF, 0.5);

	const double r_max = file_get_key(stdin, "r_max", r_min, INF, r_min + 30.0);
	const double R_max = file_get_key(stdin, "R_max", R_min, INF, R_min + 100.0);

	const double r_step = (r_max - r_min)/as_double(rovib_grid_size);
	const double R_step = (R_max - R_min)/as_double(scatt_grid_size);

	/* NOTE: 1 == a, 2 == b, 3 == c */
	const char arrang = 96 + (int) file_get_key(stdin, "arrang", 1.0, 3.0, 1.0);

	/* NOTE: 0 = false = not use, 1 = true = use */
	const bool use_omp = (bool) file_get_key(stdin, "use_omp", 0.0, 1.0, 1.0);

	const mass_case a = init_atomic_masses(stdin, arrang, 'a', cpu_id);

	const int lambda_max = (int) file_get_key(stdin, "lambda_max", 0.0, INF, 20.0);

	const int lambda_step = init_lambda_step(arrang);

	const int max_task = scatt_grid_size*rovib_grid_size*(lambda_max/lambda_step + 1);

	task *list = allocate(max_task, sizeof(struct task), true);
	...
	for (int n = n_min; n <= n_max; ++n)
	{
		...
		list[n].result = pes_legendre_multipole(arrang, list[n].lambda, list[n].r, list[n].R);
		const double end_time = wall_time();

		print(thread_id(), max_task, R, end_time - start_time);
